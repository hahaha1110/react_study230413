...
const paragraph = D.makeArray(2).map(D.randomParagraphs).join('/n/n')
const summery = D.makeArray(3).map(D.randoSentence).join('/n')

export default function TextsTest() {
  return (
    <div>
      <Title className="text-blue-600">{D.randomTitleText()}</Title>
      <Subtitle className="text-blue-400">{D.randoSentence()}</Subtitle>
